...
def single_news(news_id):
    ...
    news_list = News.query.filter(News.id != news_id, News.text.isnot(None)).order_by(News.published.desc()).all()

    if not my_news:
        abort(404)

    return render_template('news/single_news.html', page_title=my_news.title, news=my_news, news_list=news_list)
